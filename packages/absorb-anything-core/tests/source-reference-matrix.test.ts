...
async function tempDir(): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "absorb-source-ref-"));
  tempRoots.push(root);
  return root;
}
...
function sourcesRelativeFor(mode: "standalone" | "overlay"): string {
  return mode === "overlay" ? path.join(".absorb", "sources") : "sources";
}
...
describe("source reference resolution", () => {
  ...
  it("records a relative workspace path and resolves it back", async () => {
    ...
    await initFramework({ target: home, name: "Shared Research", standalone: true, agents: false });
    await initFramework({ target: consumer, name: "Product", standalone: true, agents: false });
    await copiedSource(home, "qiskit", registry);
...
describe("broken source references", () => {
  ...
  it("unlinks a broken reference without touching anything else", async () => {
    ...
    await initFramework({ target: home, name: "Home", standalone: true, agents: false });
    await initFramework({ target: consumer, name: "Consumer", standalone: true, agents: false });
    await copiedSource(home, "qiskit", registry);
    await linkSource({
      root: consumer,
      workspace: home,
...
describe("clone registry", () => {
  ...
  it("advises linking instead of a second clone, then proceeds anyway", async () => {
    ...
    expect(notices.some((notice) => notice.includes(home))).toBe(true);
    expect(notices.some((notice) => /absorb link/.test(notice))).toBe(true);
