...
/** The one older tuple `absorb update` migrates in place. */
...
export class WorkspaceCutoverRequiredError extends FrameworkError {
  ...
  constructor(observed: string) {
    ...
    const locator = migratable
      ? `absorb-update:${observed}->${REQUIRED_WORKSPACE_TUPLE}`
      : `absorb-cutover:${observed}->${REQUIRED_WORKSPACE_TUPLE}`;
    super(
      migratable
        ? `Workspace migration required: observed ${observed}; required ${REQUIRED_WORKSPACE_TUPLE}; run \`absorb update\`; locator ${locator}`
...
export class SystemsRegistryCutoverRequiredError extends FrameworkError {
  ...
  constructor(observedRegistrySchema: number | "unknown") {
    ...
    const locator = `absorb-cutover:${observed}->${required}`;
    super(
