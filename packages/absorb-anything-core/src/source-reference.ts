...
export async function readSourceReferenceRecord(file: string): Promise<SourceReferenceRecord> {
  let parsed: unknown;
  try {
    parsed = parseYaml(await readUtf8File(file, "source reference"));
  } catch (error) {
...
async function readTargetWorkspace(root: string): Promise<TargetWorkspaceRead> {
  ...
  try {
    ...
    if (!manifest || !layout) {
      return { ok: false, reason: `target is not a compatible workspace: ${root}` };
    }
    ...
    if (error instanceof WorkspaceCutoverRequiredError) {
      return {
        ok: false,
        reason: `target workspace ${root} is on an older format; run \`absorb update\` there first`,
      };
    }
    return {
      ok: false,
...
export async function resolveSourceReference(
  ...
  if (await exists(path.join(entryRoot, SOURCE_REFERENCE_FILE))) {
    return {
      ok: false,
      reason: `'${alias}' in ${workspace.value.root} is itself a reference; re-run \`absorb link\` so this shell records the workspace that owns it`,
    };
  }
...
export async function relinkSuggestions(input: {
  ...
  return candidates.map(
    (candidate) =>
      `absorb link ${candidate.workspace} ${candidate.alias} --alias ${input.localAlias}`,
  );
}
...
async function consumerSourcesRelative(root: string): Promise<string> {
  ...
  const layout = resolveWorkspaceLayout(manifest);
  if (!manifest || !layout) {
    throw new FrameworkNotFoundError(`No compatible workspace at ${root}.`);
  }
...
async function workspaceFromRegistry(
  ...
  if (candidates.length === 0) {
    throw new FrameworkNotFoundError(
      `no workspace given and the clone registry knows no home for '${alias}'; pass the target workspace: absorb link <target-workspace> ${alias}`,
    );
  }
  ...
  if (candidates.length > 1 || !first) {
    throw new FrameworkError(
      [
        ...
        ...candidates.map((candidate) => `  absorb link ${candidate.workspace} ${alias}`),
      ].join("\n"),
    );
  }
  return {
...
async function homeOriginUri(entryRoot: string): Promise<string> {
  try {
    const parsed = parseYaml(
      await readUtf8File(path.join(entryRoot, SOURCE_LINEAGE_FILE), "Source lineage"),
    );
    if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
