...
function requireManifestPresent(
  ...
  if (!manifest) {
    throw new FrameworkNotFoundError(
      `No workspace manifest found at ${path.join(root, MANAGED_DIR, "manifest.json")}.`,
    );
  }
  return manifest;
}
...
function layoutForManifest(manifest: FrameworkManifest) {
  ...
  if (!layout) {
    throw new FrameworkNotFoundError("Workspace layout could not be resolved.");
  }
  return layout;
}
...
async function readYamlFile<T>(file: string): Promise<T> {
  const parsed = parseYaml(await readUtf8File(file, "YAML file"));
...
async function writeSourceCard(
  ...
  const lines = [
    ...
    `- Content: ${contentDir}/ (${lineage.content_mode === "checkout" ? "checkout-backed, moved by `absorb sync`" : "copied content, replaced by `absorb import`"})`,
...
async function existingHomeAdvisories(
  ...
  return candidates
    ...
    .map(
      (candidate) =>
        `Advisory: '${candidate.alias}' in ${candidate.workspace} is already a home for this origin. \`absorb link ${candidate.workspace} ${candidate.alias}\` shares that one checkout, ledger, and brief instead of starting a second.`,
    );
}
