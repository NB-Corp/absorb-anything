...
export async function managedFilesPath(root: string): Promise<string> {
  return path.join((await resolveEnvelopeContext(root)).path, "managed-files.json");
}
...
export async function loadManagedFiles(root: string): Promise<ManagedFilesReceipt> {
  const file = await managedFilesPath(root);
