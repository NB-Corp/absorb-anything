...
import { projectLogicalPath } from "./envelope.js";
 ...
 */
async function applyWorkspaceMigrationUnlocked(options: {
  readonly root: string;
  readonly now?: Date;
  ...
  const context: WorkspaceMigrationContext = {
    root: analysis.root,
    sourcesRelative: projectLogicalPath(
      rawSourcesRelative(manifest),
      (await resolveEnvelopeContext(analysis.root)).directory,
    ),
  ...
  await writeFile(
    path.join((await resolveEnvelopeContext(analysis.root)).path, "manifest.json"),
...
export async function applyWorkspaceMigration(options: {
  ...
  return withWorkspaceMutationCoordination(root, () =>
    applyWorkspaceMigrationUnlocked({ ...options, root }),
  );
}
...
const zonePurposeVocabularyStep: WorkspaceMigrationStep = {
  ...
  run: async (context) => {
    ...
    await writeFile(
      path.join((await resolveEnvelopeContext(context.root)).path, "manifest.json"),
